...
'''
main idea:
1.先对点云做分段处理，通过第一段的聚类找出点数最多的一个类别，拟合直线，对所有点云做旋转
2.做旋转后的点云基本垂直于x轴，这样做的好处是防止产生的最后一段点云有上三角现象
3.把旋转后的点云重新分段，然后旋转，聚类。
4.concat的时候采用了zip和sorted，以及zip longest的函数的调用。
'''
class lane_ID:
    def __init__(self):
        self.min_points_x = .0
    ...
    def split_PC(self,points,per_size):
        ...
        grid_num = math.ceil((max_y - min_y)/per_size)
        grid_PC  = []
    ...
    def rvis_All_Labels(self,points):
        ...
        # open3d.visualization.RenderOption.point_size = 100
        # open3d.visualization.RenderOption.show_coordinate_frame = True

        w = open3d.geometry.TriangleMesh.create_coordinate_frame(size=100, origin=[0, 1, 0])
        seg.append(w)
        open3d.visualization.draw_geometries(seg)
...
if __name__ == '__main__':
    ...
    for file in os.listdir(data_dir):
        ...
        # points = a.read_pcd('/home/ral/BD_Work/Lane_seg/pcd_deal/20200603095539_YIFEIYANG_SHANGHAI_DSX116_LiDAR_2_0578.pcd')
        # a.vis(points)
        fir_pice_PC_size = 25
        grids_pc = a.split_PC(points,fir_pice_PC_size)

        labels = a.cluster(grids_pc[0])
        n_clusters_ = len(set(labels)) - (1 if -1 in labels else 0)
        max = 0
        id = 0
        labels_points_id = []
        for j in range(n_clusters_):
            ...
            if len(labels_points[0]) > max:
                ...
                max = len(labels_points[0])
        # vis(points[labels_points_id,:])
        K, intercept_ = a.ransac_line(grids_pc[0][labels_points_id, :2])
        # rotated raw pc data and split
        point_Rotated_Pc = a.rotate_pc_(K, intercept_, points)
        min_points_x, min_points_y = np.min(point_Rotated_Pc[:, :2], axis=0)
        point_move_ = point_Rotated_Pc - np.append(np.min(point_Rotated_Pc[:, :2], axis=0), 0)
        grids_pc = a.split_PC(point_move_, fir_pice_PC_size)
        for k in range(0,len(grids_pc)):
            grids_pc[k] = grids_pc[k] + np.array([min_points_x, min_points_y,0])

        grid_ = grids_pc[0].copy()
        grid_[:,1] = 0
        labels = a.cluster(grid_)
        all_pices_labels.append(labels)
        all_pc_rotated.append(grid_)


        for i in range(1,len(grids_pc)):
            # a.vis(grids_pc[i])
            ...
            # a.labels_rvz(labels,grids_pc[i])
